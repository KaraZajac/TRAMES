...
show a gradient of $\num{1.61}\times$ against $\num{1.20}\times$ for Flock, whose
installations are five-sixths of the network. Per unit deployed, non-Flock cameras reach
\num{2.46}$\times$ as many commuters. Every vendor-resolved gradient likewise dissolves
under within-county ranking.
...
encounters). Installation shares are of the \num{119132} cameras that contribute cones,
so they differ slightly from the \num{120838}-node denominator of \Cref{tab:tags}. Flock
holds five-sixths of installations and supplies two-thirds of exposure; per unit deployed,
non-Flock cameras reach \num{2.46}$\times$ as many commuters.}
\label{tab:vendorexp}
\begin{tabular}{lrrrrr}
\toprule
Set & Installations & Exposures & Exposures/\num{1000} cameras & Routes $\geq$1 & Evaded \\
\midrule
Flock Safety  & \SI{83.0}{\percent} & \SI{67.9}{\percent} & \num{579}  & \SI{65.6}{\percent} & \SI{95.0}{\percent} \\
Other vendor  & \SI{12.7}{\percent} & \SI{25.6}{\percent} & \textbf{\num{1423}} & \SI{27.4}{\percent} & \SI{92.1}{\percent} \\
No vendor tag & \SI{4.3}{\percent}  & \SI{6.5}{\percent}  & \num{1078} & \SI{11.6}{\percent} & \SI{97.1}{\percent} \\
\bottomrule
\end{tabular}
\end{table}

Flock Safety holds \SI{83.0}{\percent} of mapped installations but supplies only
\SI{67.9}{\percent} of measured commute exposure. Per unit deployed, non-Flock cameras
generate \textbf{\num{2.46}$\times$} more exposure (\Cref{tab:vendorexp}). The mechanism
...
\midrule
Flock Safety  & \num{1.20} & \num{0.95} & CIs disjoint \\
Other vendor  & \textbf{\num{1.61}} & \num{1.18} & \textbf{CIs disjoint} \\
No vendor tag & \num{1.18} & \num{1.10} & indistinguishable \\
\midrule
All vendors   & \num{1.29} & \num{1.02} & CIs disjoint \\
\bottomrule
\end{tabular}
\end{table}
...
quartile of Black population at \num{1.61}$\times$ the rate of the lowest, against
\num{1.20}$\times$ for Flock. The disparity is roughly a third larger in infrastructure
...
And the discipline of \Cref{sec:results:coverage} applies here without exception. Under
within-county ranking every vendor-resolved gradient loses significance---Flock to
\num{0.95}$\times$, other vendors to \num{1.18}$\times$, untagged to \num{1.10}$\times$,
all with overlapping intervals. The non-Flock figure remains directionally positive but is
no longer distinguishable. These are between-county phenomena exactly as the pooled result
was. Holding RQ5 to a weaker evidentiary standard than RQ3 would have let a between-county
artefact return wearing a vendor label.
...
overlapping). That was wrong, and the error is instructive. The route-geometry sidecar is
...
moved Flock from ``no gradient'' to a disjoint \num{1.20}$\times$ and would have supported
...
corridors laid down decades before \alpr existed. Regulation aimed only at the vendor
